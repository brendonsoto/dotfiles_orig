# Meine Zettlekasten

What is zettlekasten?
https://zettelkasten.de/posts/overview/

From Software Lead Weekly (SWLW) #455
@chrishlad: Grab a notebook. Create a section titled “I Don’t Know”. Every time you don’t understand something, write it down there. After a week, you may have 50 concepts or questions. 1) Google the simple ones 2) Ask a friend for the hard ones. This is exponential learning.

# Questions / Things to explore
- Vue:
  - state management
  - forms
- Figma

- Haskell:
  - STATE MONAD
  - Understand Xmonad
  - How to print (hello world)
  - How to take std input
  - Why can lazy evaluation lead to performance headaches? It's touted as a major point of Haskell.

- Language Server Protocol + Implementation
- Cross-Site Request Forgery

- Difference between syntax and grammar?
For more context, Haskell, why does the syntax not bother people?
I feel like it's the language, the words to describe functions itself.
The ideas are clearer to understand, it's more like English.
But what do you call the words?

- Haxe?

# Generated Tags

## accessibility

- [accessibility](accessibility)

## adhd

- [adhd--sachs](adhd--sachs)

## architecture

- [micro-frontends](micro-frontends)

## article

- [article-phoenix-blog](article-phoenix-blog)
- [different-states-of-priority-recruit-different-neural-representations-in-visual-working-memory](different-states-of-priority-recruit-different-neural-representations-in-visual-working-memory)
- [shared-mechanisms-underlie-the-control-of-working-memory-and-attention](shared-mechanisms-underlie-the-control-of-working-memory-and-attention)

## audio-production

- [attenuation](attenuation)
- [compression](compression)
- [console-1](console-1)
- [riding-faders](riding-faders)
- [studio-one](studio-one)

## bitwig

- [bitwig](bitwig)

## book

- [agile-web-development-with-rails-6](agile-web-development-with-rails-6)
- [Art of Problem Solving Vol 1](art-of-problem-solving-vol-1#Art of Problem Solving Vol 1)
- [Docker in Practice, Second Edition](docker-in-practice#Docker in Practice, Second Edition)

## books

- [a-type-of-programming](a-type-of-programming)
- [vimscript-the-hard-way](vimscript-the-hard-way)

## canvas

- [Canvas](canvas#Canvas)

## cardinality

- [cardinality](cardinality)

## cognitive-load

- [cognitive-load](effective-mental-models-for-code-and-systems#cognitive-load)

## course

- [design-for-developers](design-for-developers)

## css-quirks

- [Spacing](spacing#Spacing)

## database

- [postgresql-basics](postgresql-basics)
- [postgresql-install](postgresql-install)

## datatypes

- [Parameterized Types](parameterized-types#Parameterized Types)
- [product-types](product-types)
- [sum-types](sum-types)
- [type-alias](type-alias)

## daw

- [bitwig](bitwig)

## design

- [addy-osmani-design-patterns](addy-osmani-design-patterns)
- [design-for-developers](design-for-developers)

## digitone

- [digitone](digitone)

## distillery

- [elixir-distillery](elixir-distillery)

## docker

- [Docker](docker#Docker)
- [Docker in Practice, Second Edition](docker-in-practice#Docker in Practice, Second Edition)

## elektron

- [digitone](digitone)

## elixir

- [ecto](ecto)
- [elixir-agents](elixir-agents)
- [elixir-applications](elixir-applications)
- [elixir-behaviors](elixir-behaviors)
- [elixir-detecting-bottlenecks](elixir-detecting-bottlenecks)
- [elixir-distillery](elixir-distillery)
- [elixir](elixir-genServer#elixir)
- [elixir-macros](elixir-macros)
- [OTP](elixir-otp#OTP)
- [elixir-plug](elixir-plug)
- [elixir-pow](elixir-pow)
- [elixir-quirks](elixir-quirks)
- [elixir-releases](elixir-releases)
- [elixir-supervisors](elixir-supervisors)
- [elixir-tasks](elixir-tasks)
- [elixir-testing-gotchas](elixir-testing-gotchas)
- [elixir-to-organize-later](elixir-to-organize-later)
- [elixir-umbrella-projects](elixir-umbrella-projects)
- [phoenix-conventions](phoenix-conventions)
- [phoenix-general](phoenix-general)

## flask

- [Flask](flask#Flask)

## fonts

- [fonts](install-fonts-unix#fonts)

## front-end

- [addy-osmani-design-patterns](addy-osmani-design-patterns)
- [computational-vs-presentational-components](computational-vs-presentational-components)
- [frontend-masters-vuejs](frontend-masters-vuejs)
- [micro-frontends](micro-frontends)

## front-end-masters

- [frontend-masters-vuejs](frontend-masters-vuejs)

## game-development

- [Creating a Game with JavaScript](creating-a-game-with-javascript#Creating a Game with JavaScript)
- [Slime Volleyball](slime-volleyball#Slime Volleyball)

## guitar

- [guitar-prac](guitar-prac)

## haskell

- [a-type-of-programming](a-type-of-programming)
- [algebraic-data-types](algebraic-data-types)
- [cardinality](cardinality)
- [handle](handle)
- [haskell](haskell)
- [haskell-applicatives](haskell-applicatives)
- [haskell-compiling](haskell-compiling)
- [haskell-extended-guide](haskell-extended-guide)
- [haskell-functors](haskell-functors)
- [haskell-installation](haskell-installation)
- [haskell-lazy-evaluation](haskell-lazy-evaluation)
- [haskell-monoid](haskell-monoid)
- [haskell-semigroup](haskell-semigroup)
- [haskell-survival-guide](haskell-survival-guide)
- [haskell-types](haskell-types)
- [ihp](ihp)
- [kinds](kinds)
- [newtype](newtype)
- [Parameterized Types](parameterized-types#Parameterized Types)
- [product-types](product-types)
- [sum-types](sum-types)
- [type-alias](type-alias)
- [type-classes](type-classes)

## ideas

- [Command Line Dictionary](command-line-dictionary#Command Line Dictionary)
- [daw-idea](daw-idea)
- [Megaman Tribute](megaman-tribute#Megaman Tribute)
- [whiteboard-app](whiteboard-app)

## javascript

- [Canvas](canvas#Canvas)
- [frontend-masters-production-vue](frontend-masters-production-vue)
- [frontend-masters-vuejs](frontend-masters-vuejs)
- [javascript](frontend-masters-vuejs#javascript)
- [javascript-chaining-recipe](javascript-chaining-recipe)
- [js-lock-files](js-lock-files)
- [node-gyp-issue](node-gyp-issue)
- [Socket IO](socket.io#Socket IO)
- [typescript-with-babel](typescript-with-babel)
- [vue-school--masterclass](vue-school--masterclass)

## jeopardy

- [dumb-jeopardy](dumb-jeopardy)

## jinja

- [Jinja](jinja#Jinja)

## languages

- [C++](c++#C++)
- [Elixir](elixir#Elixir)
- [haskell](haskell)
- [language-server-protocol](language-server-protocol)
- [Python](python#Python)

## lessons-learned

- [lessons-learned--renting-apartments](lessons-learned--renting-apartments)

## math

- [Conjugate Radical](conjugate-radical#Conjugate Radical)

## mental-health

- [adhd--sachs](adhd--sachs)
- [vyvanse](vyvanse)

## mental-models

- [mental-models](effective-mental-models-for-code-and-systems#mental-models)

## music

- [bitwig](bitwig)
- [digitone](digitone)
- [guitar-prac](guitar-prac)
- [sysex](sysex)

## news

- [nytimes-trump-taxes](nytimes-trump-taxes)

## nix

- [niv](niv)
- [nix](nix)
- [nix--scratchpad](nix--scratchpad)
- [nix-env](nix-env)
- [nix-shell](nix-shell)

## phoenix

- [article-phoenix-blog](article-phoenix-blog)
- [elixir-pow](elixir-pow)
- [phoenix-alter-schema-after-gen](phoenix-alter-schema-after-gen)
- [phoenix-authentication](phoenix-authentication)
- [phoenix-channels](phoenix-channels)
- [phoenix-conventions](phoenix-conventions)
- [phoenix-cookbook](phoenix-cookbook)
- [phoenix-general](phoenix-general)
- [phoenix-liveview](phoenix-liveview)
- [phoenix-models](phoenix-models)
- [phoenix-protocols](phoenix-protocols)
- [phoenix-routes](phoenix-routes)
- [phoenix-seeding-data](phoenix-seeding-data)
- [phoenix-views](phoenix-views)

## politics

- [nytimes-trump-taxes](nytimes-trump-taxes)

## postgresql

- [postgresql-basics](postgresql-basics)
- [postgresql-install](postgresql-install)

## problems-and-solutions

- [ruby--bundler--shared-mime-types](ruby--bundler--shared-mime-types)
- [ruby-bundle-dep-version-not-found](ruby-bundle-dep-version-not-found)
- [rvm--allow-login-shell](rvm--allow-login-shell)

## programming

- [C++](c++#C++)
- [call-by-value](call-by-value)
- [programming](effective-mental-models-for-code-and-systems#programming)
- [programming](frontend-masters-vuejs#programming)
- [language-server-protocol](language-server-protocol)
- [web-app-architecture](web-app-architecture)
- [zsh-scripting](zsh-scripting)

## projects

- [Mobile Controllers](mobile-controllers#Mobile Controllers)
- [Pass to Bitwarden](pass2bitwarden#Pass to Bitwarden)

## python

- [Flask](flask#Flask)
- [Jinja](jinja#Jinja)
- [Python](python#Python)

## rails

- [agile-web-development-with-rails-6](agile-web-development-with-rails-6)
- [hartl-rails-6-ch1](hartl-rails-6-ch1)

## ruby

- [agile-web-development-with-rails-6](agile-web-development-with-rails-6)
- [hartl-rails-6-ch1](hartl-rails-6-ch1)
- [rake](rake)
- [ruby--bundler--shared-mime-types](ruby--bundler--shared-mime-types)
- [ruby--idioms](ruby--idioms)
- [ruby-bundle-dep-version-not-found](ruby-bundle-dep-version-not-found)
- [rvm--allow-login-shell](rvm--allow-login-shell)

## science

- [different-states-of-priority-recruit-different-neural-representations-in-visual-working-memory](different-states-of-priority-recruit-different-neural-representations-in-visual-working-memory)
- [shared-mechanisms-underlie-the-control-of-working-memory-and-attention](shared-mechanisms-underlie-the-control-of-working-memory-and-attention)

## socket.io

- [Socket IO](socket.io#Socket IO)

## stupid

- [dumb-jeopardy](dumb-jeopardy)

## systems

- [systems](effective-mental-models-for-code-and-systems#systems)

## tech

- [computational-vs-presentational-components](computational-vs-presentational-components)

## thinking

- [decision-matrix](decision-matrix)
- [second-order-thinking](second-order-thinking)

## to-sort

- [how-to-speak](how-to-speak)

## types

- [typescript-casting](typescript-casting)
- [typescript-discriminated-unions](typescript-discriminated-unions)
- [typescript-intersection-types](typescript-intersection-types)
- [typescript-types](typescript-types)
- [typescript-union-types](typescript-union-types)

## typescript

- [typescript-aliases](typescript-aliases)
- [typescript-casting](typescript-casting)
- [typescript-config](typescript-config)
- [typescript-dictionary](typescript-dictionary)
- [typescript-discriminated-unions](typescript-discriminated-unions)
- [typescript-flags](typescript-flags)
- [typescript-good-to-know](typescript-good-to-know)
- [typescript-interfaces](typescript-interfaces)
- [typescript-intersection-types](typescript-intersection-types)
- [typescript-node-modules](typescript-node-modules)
- [typescript-object-properties](typescript-object-properties)
- [typescript-overloaded-functions](typescript-overloaded-functions)
- [typescript-questions](typescript-questions)
- [typescript-quickly-get-type](typescript-quickly-get-type)
- [typescript-tuple](typescript-tuple)
- [typescript-typed-object-keys](typescript-typed-object-keys)
- [typescript-types](typescript-types)
- [typescript-union-types](typescript-union-types)
- [typescript-with-babel](typescript-with-babel)

## unix

- [unix](install-fonts-unix#unix)

## video-game

- [Creating a Game with JavaScript](creating-a-game-with-javascript#Creating a Game with JavaScript)
- [Megaman Tribute](megaman-tribute#Megaman Tribute)
- [Mobile Controllers](mobile-controllers#Mobile Controllers)
- [Slime Volleyball](slime-volleyball#Slime Volleyball)

## vim

- [vim-handy](vim-handy)

## vue

- [frontend-masters-production-vue](frontend-masters-production-vue)
- [frontend-masters-vuejs](frontend-masters-vuejs)
- [vue-school--masterclass](vue-school--masterclass)
- [vuex](vuex)

## vue-school

- [vue-school--masterclass](vue-school--masterclass)

## what-to-do

- [what-to-do--car-crash](what-to-do--car-crash)

## working-memory

- [different-states-of-priority-recruit-different-neural-representations-in-visual-working-memory](different-states-of-priority-recruit-different-neural-representations-in-visual-working-memory)
- [shared-mechanisms-underlie-the-control-of-working-memory-and-attention](shared-mechanisms-underlie-the-control-of-working-memory-and-attention)
